...
results_file = dependency_glob('*_eotest_results_flat_pairs.fits')[0]

# Infer the sensor_id from the results filename.
sensor_id = os.path.basename(results_file).split('_')[0]

shutil.copy(results_file, '%s_eotest_results.fits' % sensor_id)

plots = sensorTest.EOTestPlots(sensor_id, results_file=results_file)

fe55_file = dependency_glob('%s_psf_results*.fits' % sensor_id)[0]
plots.fe55_dists(fe55_file=fe55_file)
pylab.savefig('%s_fe55_dists.png' % sensor_id)

ptc_file = dependency_glob('%s_ptc.fits' % sensor_id)[0]
plots.ptcs(ptc_file=ptc_file)
pylab.savefig('%s_ptcs.png' % sensor_id)

detresp_file = dependency_glob('%s_det_response.fits' % sensor_id)[0]
...
qe_file = dependency_glob('*%s_QE.fits' % sensor_id)[0]
plots.qe(qe_file=qe_file)
pylab.savefig('%s_qe.png' % sensor_id)

xtalk_file = dependency_glob('*%s_xtalk_matrix.fits' % sensor_id)[0]
